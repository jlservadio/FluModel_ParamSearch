...
setwd('~/work/Param_Search/R16R_L_nI')
...
file.name = 'Code/ModelCode_R16R_L_nI.txt'
...
while (cur.time < tot.time) {
	...
	it.pars = list()
	it.pars = mclapply(c(1:(n.par*100)), function(x) {
...
if (!is.null(dim(keep.pars))) { save(keep.pars, file = paste('Model_Results/Params_', Sys.Date(), '_of_', it*length(it.pars), 'a.Rdata', sep = '')) }
