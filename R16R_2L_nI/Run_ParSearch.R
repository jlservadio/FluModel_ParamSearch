...
setwd('~/work/Param_Search/R16R_2L_nI')
...
file.name = 'Code/ModelCode_R16R_2L_nI.txt'
...
n.par = 10
tot.time = 60*60*47.85; tot.time = 60*60*24
...
while (cur.time < tot.time) {
	...
	it.pars = list()
	it.pars = mclapply(c(1:(n.par*1000)), function(x) {
...
#	new.params = c(new.params, round(runif(1, 130, 365)))
	...
	if (!is.null(dim(keep.pars)) && nrow(keep.pars) > 1000) { break }	
}
